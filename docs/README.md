...
**New here?** Start with [Getting Started](getting-started.md) — about fifteen
...
| [Glossary](glossary.md) | Every term defined once — **node vs. task**, **asset vs. instance** |
| [SimFlow Component](simflow-component.md) | The component that runs a flow: start modes, save, networking, controls |
| [Blackboard](blackboard.md) | The per-flow key/value store; well-known keys |
...
The part of SimFlow that lets a flow say *"the foam extinguisher"* instead of
*"that actor over there"*.

| Page | What it covers |
|---|---|
| [Identity System](identity.md) | What an object **is**: tags, display names, held state, `Identity Id` |
| [Actor Query](actor-query.md) | *"Which object does this task mean?"* — and how near misses are graded |
| [Zones](zones.md) | Named volumes that report what is inside, and when it has settled |

---

## Nodes Reference

A **node** controls *where execution goes*. → **[Node Reference index](nodes/README.md)**
...
| **Tasks** | [Task](nodes/task.md) — *the node designers use most* |
...
A **task** is the *work* that happens inside a [Task node](nodes/task.md).
→ **[Task Reference index](tasks/README.md)**
...
Complete scenarios showing several nodes and tasks working together.
...
**[Troubleshooting](troubleshooting.md)** — symptoms first, organised by what you
are seeing.

The two tools to reach for before anything else:
...
whole blackboard. And filter the Output Log by **`LogSimFlow`**: most
...
Every node, task and component page follows the same template, so you can jump
straight to the section you need:

1. **Overview / Purpose** — what problem it solves
2. **Field-by-field breakdown** — type, default, required, meaning
3. **Behaviour when left empty or misconfigured** — what actually happens
4. **Dependencies** — what it needs, and what needs it
5. **Example use case** — step-by-step setup
6. **Common pitfalls** — symptom → cause
